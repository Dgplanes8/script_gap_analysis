...
DROP POLICY IF EXISTS "Users can see their brief history" ON public.creative_brief_requests;
CREATE POLICY "Users can see their brief history" ON public.creative_brief_requests
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own requests" ON public.creative_brief_requests;
CREATE POLICY "Users can insert their own requests" ON public.creative_brief_requests
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Service role can manage anonymous rows" ON public.creative_brief_requests;
